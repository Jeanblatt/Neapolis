 ...
 * navigateur confirmée, dans un chunk JS séparé chargé uniquement ici.
 */
export default function HeroSceneLoader() {
  const [show3D, setShow3D] = useState(false);

  useEffect(() => {
    // Détection unique de capacité navigateur (WebGL, préférence de mouvement) — pas un dérivé de state React.
  ...
  return (
    <div className="relative mx-auto h-[420px] w-full max-w-lg sm:h-[480px]">
      {show3D ? <HeroScene /> : <HeroStaticVisual />}
    </div>
  );
}
